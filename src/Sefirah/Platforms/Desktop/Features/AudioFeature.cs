...
public sealed class AudioFeature(
    ILogger<AudioFeature> logger,
    ILoggerFactory loggerFactory,
    ISessionManager sessionManager,
    IDeviceManager deviceManager) : IAudioFeature, IAsyncDisposable
{
    ...
    public Task HandleAudioActionAsync(AudioAction action)
    {
        switch (action.ActionType)
        {
            case AudioActionType.DefaultDevice:
                pulseAudioClient.SetDefaultSink(action.Source);
                break;
            case AudioActionType.VolumeUpdate when action.Value.HasValue:
                pulseAudioClient.SetVolume(
                    action.Source,
                    Convert.ToSingle(action.Value.Value));
                break;
            case AudioActionType.ToggleMute:
                pulseAudioClient.ToggleMute(action.Source);
                break;
        }
