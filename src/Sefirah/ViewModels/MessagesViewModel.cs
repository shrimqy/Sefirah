...
public sealed partial class MessagesViewModel : BaseViewModel
{
    ...
    private ObservableCollection<SmsConversation>? activeConversationsReference;
    public ObservableCollection<SmsConversation>? Conversations => activeConversationsReference;

    // Search functionality
    private readonly ObservableCollection<SmsConversation> searchResults = [];
    public ObservableCollection<SmsConversation> SearchResults => searchResults;
    ...
    private readonly ObservableCollection<TextMessage> conversationMessages = [];
    public ObservableCollection<TextMessage> Messages => conversationMessages;
    ...
    private readonly ObservableCollection<string> newConversationAddresses = [];
    public ObservableCollection<string> NewConversationAddresses => newConversationAddresses;

    [ObservableProperty]
    public partial string MessageText { get; set; } = string.Empty;

    // Phone numbers from active device
    private readonly ObservableCollection<PhoneNumber> phoneNumbers = [];
    public ObservableCollection<PhoneNumber> PhoneNumbers => phoneNumbers;
    ...
    private async Task LoadConversationsForActiveDevice()
    {
        if (ActiveDevice == null)
        {
            activeConversationsReference = null;
        ...
        try
        {
            ...
            activeConversationsReference = smsHandlerService.GetConversationsForDevice(ActiveDevice.Id);
            OnPropertyChanged(nameof(Conversations));

            logger.LogInformation("Loaded {Count} conversations for device: {DeviceId}", 
                activeConversationsReference.Count, ActiveDevice.Id);
        }
        catch (Exception ex)
        {
    ...
    private async void LoadMessagesForSelectedConversation()
    {
        ...
        try
        {
            ...
            {
                var existingIds = new HashSet<long>(conversationMessages.Select(m => m.UniqueId));
            ...
            logger.LogInformation("Loaded {Count} messages for conversation: {ThreadId}", 
                conversationMessages.Count, SelectedConversation.ThreadId);
        }
        catch (Exception ex)
        {
    ...
    public void SearchConversations(string searchText)
    {
        ...
        if (string.IsNullOrWhiteSpace(searchText) || activeConversationsReference == null)
            return;

        var filtered = activeConversationsReference
    ...
    private void OnConversationsUpdated(object? sender, string deviceId)
    {
        if (ActiveDevice?.Id == deviceId)
        {
            dispatcher.EnqueueAsync(() =>
            {
                ...
                {
                    var updatedConversation = activeConversationsReference?.FirstOrDefault(c => c.ThreadId == SelectedConversation.ThreadId);
